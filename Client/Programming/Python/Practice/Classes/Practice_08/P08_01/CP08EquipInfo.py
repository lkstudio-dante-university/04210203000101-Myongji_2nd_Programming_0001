...
class CP08EquipInfo:
	...
	def GetKinds(self):
		...
		return oKindsList[self.m_nKinds] if self.m_nKinds > CP08EquipInfo.KINDS_NONE and self.m_nKinds < CP08EquipInfo.KINDS_MAX_VAL else "알수 없음"
	...
	def GetGrade(self):
		...
		return oGradeList[self.m_nGrade] if self.m_nGrade > CP08EquipInfo.GRADE_NONE and self.m_nGrade < CP08EquipInfo.GRADE_MAX_VAL else "알수 없음"
...
# 메뉴 장비 정보
class CP08MenuEquipInfo(CP08EquipInfo):
	# 초기화
	def __init__(self, a_nKinds: int, a_nNumEquips: int, a_nGrade: int = CP08EquipInfo.GRADE_NONE):
