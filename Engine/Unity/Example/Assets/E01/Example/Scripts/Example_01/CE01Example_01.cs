 ...
 * 
 * 에셋이란?
 * - 이미지와 같은 프로그램을 제작하기 위해서 사용되는 여러 자원을 의미한다. Unity 는 이미지와 같은 파일을 인식하기 위해서
 * 자동으로 메타 파일을 생성하며 메타 파일과 리소스 파일을 묶어서 에셋이라고 지칭한다. (즉, Unity 에서 이미지와 같은 리소스를
 * 사용하기 위해서는 반드시 해당 리소스와 쌍을 이루는 메타 파일이 필요하다는 것을 알 수 있다.)
 */
namespace E01 {
	/** Example 1 */
	public partial class CE01Example_01 : CE01SceneManager {
		#region 프로퍼티
		public override string SceneName => KE01Define.G_SCENE_N_EXAMPLE_01;
		#endregion // 프로퍼티
