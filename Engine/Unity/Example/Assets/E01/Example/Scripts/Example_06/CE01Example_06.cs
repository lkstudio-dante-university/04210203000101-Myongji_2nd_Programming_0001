	...
	public partial class CE01Example_06 : CE01SceneManager {
		...
		public override void Awake() {
			...
			for(int i = 0; i < oValList01.Count; ++i) {
				Debug.Log($"{oValList01[i]}");
			}

			Debug.Log("=====> 리스트 요소 - 2 <=====");

			for(int i = 0; i < oValList02.Count; ++i) {
				Debug.Log($"{oValList02[i]}");
			}
#elif E06_DICTIONARY
			var oValDict01 = new Dictionary<string, int>();

			var oValDict02 = new Dictionary<string, int>() {
			 ...
			 */
			foreach(var stKeyVal in oValDict01) {
				Debug.Log($"[{stKeyVal.Key}]:{stKeyVal.Value}");
			}

			Debug.Log("=====> 딕셔너리 요소 - 2 <=====");

			foreach(var stKeyVal in oValDict02) {
				Debug.Log($"[{stKeyVal.Key}]:{stKeyVal.Value}");
			}
