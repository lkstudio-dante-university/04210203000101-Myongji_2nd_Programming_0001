...
namespace E01 {
	/** 카메라 제어자 */
	public class CE01CameraController_23 : CE01Component {
		#region 변수
		...
		private void SetupCameraPos(bool a_bIsImmediate = false) {
			...
			var stOffset = (m_oFollowTarget.transform.forward * -m_fDistance) + (Vector3.up * m_fHeight);
			...
			if(a_bIsImmediate) {
				...
				this.transform.position = Vector3.Lerp(this.transform.position, stNextPos, Time.deltaTime * 5.0f);
			}
