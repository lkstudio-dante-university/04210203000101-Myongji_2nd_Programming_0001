...
namespace E01 {
	/** 풀 리스트 */
	public class C01PoolList_23<T> {
		public List<T> m_oList = new List<T>();
		public Queue<T> m_oQueue = new Queue<T>();
	}

	/** 객체 풀 관리자 */
	public class CE01ObjsPoolManager_23 : CE01Component {
		#region 변수
		private Dictionary<System.Type, C01PoolList_23<object>> m_oPoolListDict = new Dictionary<System.Type, C01PoolList_23<object>>();
		#endregion // 변수

		#region 함수
		/** 객체를 활성화한다 */
		public object SpawnObj<T>(System.Func<object> a_oCreator) {
			var oPoolList = m_oPoolListDict.GetValueOrDefault(typeof(T)) ?? new C01PoolList_23<object>();
		...
		public void DespawnObj<T>(object a_oObj, System.Action<object> a_oCallback) {
			// 객체 풀이 존재 할 경우
			if(m_oPoolListDict.TryGetValue(typeof(T), out C01PoolList_23<object> oPoolList)) {
