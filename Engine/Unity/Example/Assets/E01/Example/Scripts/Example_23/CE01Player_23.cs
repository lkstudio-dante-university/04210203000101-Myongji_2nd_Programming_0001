...
namespace E01 {
	/** 플레이어 */
	public class CE01Player_23 : CE01Component {
		#region 변수
		...
		public override void Awake() {
			...
			m_oStatHandler.SetStat(EE01StatKinds_23.HP, 10);
			m_oStatHandler.SetStat(EE01StatKinds_23.ATK, 1);
			m_oStatHandler.SetStat(EE01StatKinds_23.MAX_HP, 10);
		}

		/** 상태를 갱신한다 */
		public override void Update() {
			base.Update();
			var oSceneManager = CE01SceneManager.GetSceneManager<CE01Example_23>(KE01Define.G_SCENE_N_EXAMPLE_23);
			...
			/*
			Axis 계열 메서드는 입력 장치의 입력 여부에 따라 -1 ~ 1 범위 사이 값을 반환한다. (즉, 아무런 입력도 없다면 0 을 
			반환하며 입력이 존재 할 경우 입력 방향에 따라 음수 또는 양수를 반환한다는 것을 알 수 있다.)

			단, Axis 메서드는 입력 방향에 따라 값이 서서히 변경되기 때문에 해당 값을 바로 이동 처리에 활용하면 입력이 종료 
			되어도 제어하는 물체가 일정 시간 동안 여전히 움직이는 단점이 존재한다.

			따라서, 입력에 따라 바로 반응하는 물체를 제작하기 위해서는 AxisRaw 메서드를 활용해야한다. (즉, AxisRaw 메서드는 
			입력 여부에 따라 -1, 0, 1 값 중 하나를 반환한다는 것을 알 수 있다.)
			*/
			 ...
			 * 
			 * 단, 해당 메서드는 내부적으로 공간 변환에 대한 기능을 지원하기 때문에 해당 설정에 따라 올바른 위치 정보를 
			 * 명시 할 필요가 있다. (즉, Space.Self 는 내부적으로 로컬 공간으로 변환하기 때문에 명시되는 위치 정보는 
			 * 월드 공간의 위치 정보를 명시해줘야한다는 것을 알 수 있다.)
			 */
			...
			// 입력이 없을 경우
			if(Mathf.Approximately(fVertical, 0.0f) && Mathf.Approximately(fHorizontal, 0.0f)) {
				m_oAnimation.CrossFade("Idle", 0.15f);
			} else {
				// 수직 입력이 존재 할 경우
				if(!Mathf.Approximately(fVertical, 0.0f)) {
					m_oAnimation.CrossFade((fVertical > 0.0f) ? "RunF" : "RunB", 0.15f);
				} else {
					m_oAnimation.CrossFade((fHorizontal > 0.0f) ? "RunR" : "RunL", 0.15f);
				}
			}
		}
		...
		public void OnTriggerEnter(Collider a_oCollider) {
			var oMonster = a_oCollider.GetComponentInParent<CE01Monster_23>();
		...
		/** 총알을 생성한다 */
		private CE01Bullet_23 CreateBullet(GameObject a_oOriginBullet, GameObject a_oBulletRoot) {
			var oSceneManager = CE01SceneManager.GetSceneManager<CE01Example_23>(KE01Define.G_SCENE_N_EXAMPLE_23);

			var oBullet = oSceneManager.ObjsPoolManager.SpawnObj<CE01Bullet_23>(() => {
				var oGameObj = Instantiate(a_oOriginBullet, Vector3.zero, Quaternion.identity);
				oGameObj.transform.SetParent(a_oBulletRoot.transform, false);

				return oGameObj;
			}) as GameObject;

			oBullet.SetActive(true);
			return oBullet.GetComponent<CE01Bullet_23>();
		}
		#endregion // 함수
	}
}
